...
def listen_print_loop(responses, bar_stream, ip, port, workingDir, voice_name="en-GB-Wavenet-B",personality_prompt=''):
    gpt = GPT3(prefix_prompt
               =personality_prompt)
    num_chars_printed = 0
    ...
    try:
        for response in responses:
            ...
            if not result.is_final:
                ...
                asyncio.run(read_and_send_audio_file("./WorkingDir/output.wav", ip, port))
                num_chars_printed = 0
...
def main(headset_ip="192.168.1.15",language_code = "en-US",port = 6547,intro_length=3, dirs=None,
    ...
    with BarSocketStream(RATE, CHUNK, save_to_file=audio_file_name) as stream:
        ...
        listen_print_loop(responses,stream,ip,port,dirs['txt_dir'],voice_name=voice_name,personality_prompt=personality_prompt)
