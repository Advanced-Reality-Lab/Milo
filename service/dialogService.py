...
from utils.UI.GUI import startzero, stopzero, startone, stopone
from utils import udp_comunicator, files_handler as fh, recorder
import logging
import sys
import tqdm

from utils.config_helpers import load_model, read_config_file
from utils.speech import STTConVRSelf_gcloud as STTConVRSelf, TTSConVRSelf
...
def stop_record(data,addr):
    ...
    sentence = stt.stt(os.path.join(session_dir,new_file_name),os.path.join(session_dir,new_file_name[:-4]))
...
# initializing gpt future
    ...
    audio_dir, txt_dir, gpt_dir, session_dir = fh.directory_creation(path_rel=config['saving_path'])

    audio_name = session_dir + '/record.wav'

    logger.info(f'directory names are {audio_dir} for audio, {txt_dir} for the text')
